...
class HybridPredictor:
    ...
    def prepare_gru_data(self, X_sequence: np.ndarray) -> np.ndarray:
        ...
        if len(X_sequence.shape) == 3:
            ...
            raise ValueError(
                f"Expected 3D input for GRU, got {len(X_sequence.shape)}D")
    ...
    def fit(self, X_sequence: np.ndarray, y: np.ndarray) -> Dict[str, float]:
        ...
        if len(X_sequence.shape) != 3:
            raise ValueError(
                f"Expected 3D input for X_sequence, got {len(X_sequence.shape)}D")
    ...
    def _validate_input_data(func):
        ...
        def wrapper(self, X_sequence, *args, **kwargs):
            ...
            if len(X_sequence.shape) != 3:
                raise ValueError(
                    f"Expected 3D input (samples, timesteps, features), got {len(X_sequence.shape)}D")
    ...
    def evaluate(self, X_sequence: np.ndarray, y: np.ndarray, dates: Optional[np.ndarray] = None) -> Dict[str, float]:
        ...
        information_ratio = np.mean(
            predictions - y) / tracking_error if tracking_error > 0 else 0.0
        ...
        downside_deviation = np.std(negative_returns) if len(
            negative_returns) > 0 else 0.0
        sortino_ratio = annual_return / \
            downside_deviation if downside_deviation > 0 else np.inf
        ...
        if dates is not None:
            ...
            # 30天滚动相关性
            df['rolling_corr'] = df['actual'].rolling(
                window=30).corr(df['predicted'])
    ...
    def _calculate_feature_importance(self, X_sequence: np.ndarray):
        ...
        if self.xgb_model is not None:
            ...
            self.logger.info(
                f"XGBoost特征重要性计算完成，前5个特征索引: {np.argsort(importance)[-5:]}")
        ...
        if self.gru_model is not None:
            try:
                ...
                self.feature_importance_cache['gru'] = np.ones(
                    X_sequence.shape[-1])  # 修复占位符问题
    ...
    def get_top_features(self, n_features: int = 10) -> Dict[str, np.ndarray]:
        ...
        self.logger.info(
            f"Retrieved top {n_features} features for both models")
        return result
