...
class TechnicalIndicators:
    ...
    def __init__(self, df: Optional[pd.DataFrame] = None):
        ...
        if df is not None:
            ...
            if all(col in self._df.columns for col in numeric_cols):
                ...
                raise ValueError(
                    f"Missing required numeric columns: {numeric_cols}")
    ...
    def _to_float64(self, arr: np.ndarray) -> np.ndarray:
        ...
        if arr.dtype != np.float64:
            self.logger.debug(
                f"Converting array from {arr.dtype} to float64 for TA-Lib")
    ...
    def calculate_moving_averages(self, close: np.ndarray, periods: List[int] = [5, 10, 20],
        ...
        if len(close) < max_period:
            self.logger.warning(
                f"Data too short ({len(close)} < {max_period}) for moving averages")
        ...
        for period in periods:
            if use_ema:
                result[f'EMA{period}'] = talib.EMA(
                    close, timeperiod=period).astype(np.float32)
            else:
                result[f'MA{period}'] = talib.SMA(
                    close, timeperiod=period).astype(np.float32)
        return result
    ...
    def calculate_rsi(self, close: np.ndarray, period: int = 14) -> Dict[str, np.ndarray]:
        ...
        if len(close) < period:
            self.logger.warning(
                f"Data too short ({len(close)} < {period}) for RSI")
    ...
    def calculate_macd(self, close: np.ndarray, fastperiod: int = 12,
        ...
        if len(close) < max_period:
            self.logger.warning(
                f"Data too short ({len(close)} < {max_period}) for MACD")
    ...
    def calculate_bollinger_bands(self, close: np.ndarray, period: int = 20,
        ...
        if len(close) < period:
            self.logger.warning(
                f"Data too short ({len(close)} < {period}) for Bollinger Bands")
    ...
    def calculate_adx(self, high: np.ndarray, low: np.ndarray, close: np.ndarray,
        ...
        if len(high) < period:
            self.logger.warning(
                f"Data too short ({len(high)} < {period}) for ADX")
    ...
    def calculate_momentum(self, close: np.ndarray, period: int = 10) -> Dict[str, np.ndarray]:
        ...
        if len(close) < period:
            self.logger.warning(
                f"Data too short ({len(close)} < {period}) for Momentum")
    ...
    def calculate_cci(self, high: np.ndarray, low: np.ndarray, close: np.ndarray,
        ...
        if len(high) < period:
            self.logger.warning(
                f"Data too short ({len(high)} < {period}) for CCI")
    ...
    def calculate_cmf(self, high: np.ndarray, low: np.ndarray, close: np.ndarray,
        ...
        if len(high) < period:
            self.logger.warning(
                f"Data too short ({len(high)} < {period}) for CMF")
        ...
        money_flow_multiplier = ((close_series - low_series) - (
            high_series - close_series)) / (high_series - low_series + 1e-10)
        money_flow_volume = money_flow_multiplier * volume_series
        cmf = money_flow_volume.rolling(window=period).sum(
        ) / volume_series.rolling(window=period).sum()
    ...
    def calculate_atr(self, high: np.ndarray, low: np.ndarray, close: np.ndarray, period: int = 5) -> Dict[str, np.ndarray]:
        if len(close) == 0:
            return self._empty_indicators(['ATR'])
        if len(close) < period:
            self.logger.warning(f"Data too short ({len(close)} < {period}) for ATR")
            return self._empty_indicators(['ATR'], len(close))
        high, low, close = self._to_float64(high), self._to_float64(low), self._to_float64(close)
        atr = talib.ATR(high, low, close, timeperiod=period).astype(np.float32)
        return {'ATR': atr}
    
    def calculate_vwap(self, high: np.ndarray, low: np.ndarray, close: np.ndarray, volume: np.ndarray) -> Dict[str, np.ndarray]:
        if len(close) == 0:
            return self._empty_indicators(['VWAP'])
        typical_price = (self._to_float64(high) + self._to_float64(low) + self._to_float64(close)) / 3
        vwap = np.cumsum(typical_price * self._to_float64(volume)) / np.cumsum(volume)
        return {'VWAP': vwap.astype(np.float32)}

    def calculate_ma_slope(self, close: np.ndarray, period: int = 5) -> Dict[str, np.ndarray]:
        if len(close) == 0:
            return self._empty_indicators(['MA_SLOPE'])
        if len(close) < period + 1:
            self.logger.warning(f"Data too short ({len(close)} < {period + 1}) for MA Slope")
            return self._empty_indicators(['MA_SLOPE'], len(close))
        close = self._to_float64(close)
        ma = talib.SMA(close, timeperiod=period)
        ma_slope = np.gradient(ma).astype(np.float32)
        return {'MA_SLOPE': ma_slope}
    ...
    def calculate_volume_indicators(self, volume: np.ndarray, period: int = 5) -> Dict[str, np.ndarray]:
        ...
        if len(volume) < period:
            self.logger.warning(
                f"Data too short ({len(volume)} < {period}) for volume indicators")
    ...
    def calculate_all_indicators(self, fillna_method: str = 'hybrid', min_data_points: int = 20, chunksize: int = 100) -> pd.DataFrame:
        ...
        indicators = {
            ...
            'MA_SLOPE': self.calculate_ma_slope,
        }
        ...
        def compute_group(group):
            ...
            if len(group) < min_data_points:
                self.logger.warning(
                    f"Skipping stock {group['ts_code'].iloc[0]}: insufficient data points ({len(group)} < {min_data_points})")
                return pd.DataFrame()
            ...
            # 计算基本指标
            result['amplitude'] = (
                group['high'] - group['low']) / group['close'].shift(1)
            ...
            for name, func in indicators.items():
                if name == 'MA':
                    ind = func(group['close'].values, periods=[
                               5, 10, 20], use_ema=False)
                elif name == 'RSI':
                    ...
                    ind = func(group['high'].values,
                               group['low'].values, group['close'].values)
                elif name == 'MOM':
                    ind = func(group['close'].values)
                elif name == 'CCI':
                    ind = func(group['high'].values,
                    ...
                    ind = func(group['high'].values, group['low'].values,
                               group['close'].values, group['volume'].values)
                elif name == 'CANDLESTICK':
                    ind = func(group['open'].values, group['high'].values,
                               group['low'].values, group['close'].values)
                elif name == 'VOLUME':
                    ind = func(group['volume'].values)
                elif name == 'ATR':
                    ind = func(group['high'].values, group['low'].values, group['close'].values, period=5)
                elif name == 'VWAP':
                    ind = func(group['high'].values, group['low'].values, group['close'].values, group['volume'].values)
                elif name == 'MA_SLOPE':
                    ind = func(group['close'].values, period=5)
                else:
                    continue
            ...
            # 处理NaN值
            indicator_cols = [
                col for col in result.columns if col not in required_columns]
            if fillna_method == 'hybrid':
                result[indicator_cols] = result[indicator_cols].ffill().bfill().fillna(
                    result[indicator_cols].median()).astype(np.float32)
            elif fillna_method == 'ffill':
                result[indicator_cols] = result[indicator_cols].ffill().fillna(
                    0).astype(np.float32)
            elif fillna_method == 'bfill':
                result[indicator_cols] = result[indicator_cols].bfill().fillna(
                    0).astype(np.float32)
            elif fillna_method == 'mean':
                result[indicator_cols] = result[indicator_cols].fillna(
                    result[indicator_cols].mean()).astype(np.float32)
        ...
        if 'ts_code' in self._df.columns:
            results = []
            groups = self._df.groupby('ts_code')
            group_list = [group for _, group in groups]
            for i in range(0, len(group_list), chunksize):  # 分批处理组
                chunk = group_list[i:i+chunksize]
                with ThreadPoolExecutor(max_workers=2) as executor:
                    chunk_results = list(executor.map(compute_group, chunk))
                results.extend([r for r in chunk_results if not r.empty])
                gc.collect()  # 每批后释放内存
            if not results:
        ...
        if nan_counts.sum() > 0:
            self.logger.warning(
                f"Remaining NaNs in indicators: {nan_counts[nan_counts > 0].to_dict()}")
