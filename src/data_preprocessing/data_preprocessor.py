...
from scipy.stats.mstats import winsorize  # 新导入
import psutil
...
class DataPreprocessor:
    ...
    def create_sequences(self, df: pd.DataFrame, target_col: str) -> Tuple[np.ndarray, np.ndarray, List[str], np.ndarray]:
        ...
        def process_stock(group):
            ...
            for i in range(len(group) - self.lookback_window - self.prediction_horizon + 1):
                ...
                tgt = target[i + self.lookback_window +
                ...
                group_dates_out.append(
                    group_dates[i + self.lookback_window - 1])
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            results = list(
                tqdm(executor.map(process_stock, groups), total=len(groups)))
        ...
        self.logger.info(
            f"序列诊断: 总股票 {len(groups)}, 跳过数据不足 {skipped_insufficient}, 跳过NaN序列 {skipped_nan}, 有效序列 {len(X_sequence)}")
        self.logger.info(
            f"生成序列完成: {len(X_sequence)}个样本，维度: {X_sequence.shape if X_sequence.size else 'empty'}")
    ...
    def fit(self, df: pd.DataFrame, model: Optional[LSTMLGBMPredictor] = None) -> Tuple[np.ndarray, np.ndarray, List[str], np.ndarray]:
        ...
        df[numeric_cols] = df[numeric_cols].astype('float32')  # 强制 float32
        ...
        df['signal'] = df['close'].pct_change(
            periods=self.prediction_horizon).shift(-self.prediction_horizon)
        ...
        self.logger.info(
            f"目标变量创建完成，有效样本数: {df[target_col].notna().sum()}, 均值: {self.data_info['target_mean']:.4f}, 标准差: {self.data_info['target_std']:.4f}")

        # 特征选择
        self.logger.info("开始特征选择...")
        all_features = [col for col in df.columns if col not in [
            'trade_date', 'ts_code', target_col]]
        self.logger.info(f"原始特征数量: {len(all_features)}")

        # 缺失值过滤
        missing_ratio = df[all_features].isna().mean()
        selected_features = [
            col for col in all_features if missing_ratio[col] < 0.5]
        self.logger.info(f"缺失值过滤后特征数量: {len(selected_features)}")

        # 多重共线性检测
        self.logger.info(
            f"开始多重共线性检测，输入特征数量: {len(selected_features)}, VIF阈值: 5.0")
        ...
        selected_features = vif_data[vif_data['VIF'] < 5.0]['feature'].tolist()
        ...
        self.logger.info(f"多重共线性检测完成: 最终选择 {len(self.feature_columns)} 个特征")
        self.logger.info(f"选择的特征: {self.feature_columns}")

        # 创建时序数据
        X_sequence, y, stock_codes, dates = self.create_sequences(
            df, target_col)
        ...
        for col in self.feature_columns:
            ...
            if self.outlier_detection == 'zscore':
                mean = col_data.mean()
                std = col_data.std()
                self.outlier_lower_bounds[col] = mean - \
                    self.outlier_threshold * std
                self.outlier_upper_bounds[col] = mean + \
                    self.outlier_threshold * std
        ...
        for i in range(X_sequence.shape[2]):
            ...
            for t in range(X_sequence.shape[1]):
                X_sequence[:, t, i] = winsorize(
                    X_sequence[:, t, i], limits=[0.01, 0.01])
            self.logger.info(f"特征 {feature}: 裁剪范围 [0.01, 0.01]")
        ...
        self.logger.info(
            f"目标变量异常值范围: [{self.target_outlier_lower:.4f}, {self.target_outlier_upper:.4f}]")

        y = winsorize(y, limits=[0.05, 0.05])  # 放宽目标变量裁剪
        self.logger.info(f"目标变量异常值处理: 裁剪 5% 上下限")

        # 拟合特征缩放器
        self.logger.info("拟合特征缩放器...")
        X_2d = X_sequence.reshape(-1, X_sequence.shape[2])
        self.feature_scaler.fit(X_2d)
        X_sequence_scaled = self.feature_scaler.transform(
            X_2d).reshape(X_sequence.shape)
    ...
    def transform(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[str], np.ndarray]:
        ...
        # 异常值处理：使用 winsorize 替换 clip
        for i in range(X_sequence.shape[2]):
            feature = self.feature_columns[i]
            for t in range(X_sequence.shape[1]):
                X_sequence[:, t, i] = winsorize(
                    X_sequence[:, t, i], limits=[0.01, 0.01])  # 裁剪1%极端值
    ...
    def load_preprocessor(self, path: str) -> None:
        ...
        self.target_imputation_value = save_data.get(
            'target_imputation_value', None)
